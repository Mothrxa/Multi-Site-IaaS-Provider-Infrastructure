...
router.post('/', (req, res) => {
  ...
  res.status(201).json(getDb().prepare('SELECT * FROM announcements WHERE id = ?').get(id))
})
...
router.patch('/:id', (req, res) => {
  ...
  getDb().prepare(`UPDATE announcements SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  res.json(getDb().prepare('SELECT * FROM announcements WHERE id = ?').get(req.params.id))
})
...
router.post('/:id/react', (req, res) => {
  ...
  if (existing) {
    if (existing.emoji === emoji) {
      ...
      db.prepare('UPDATE announcement_reactions SET emoji = ? WHERE id = ?').run(emoji, existing.id)
    }
  } else {
    db.prepare(
  ...
  const count = db.prepare('SELECT COUNT(*) AS n FROM announcement_reactions WHERE announcement_id = ?').get(req.params.id).n
