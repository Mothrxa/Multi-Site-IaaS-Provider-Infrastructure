...
router.post('/upload', upload.single('file'), (req, res) => {
  ...
  res.status(201).json(db.prepare('SELECT d.*, u.name AS owner_name FROM documents d JOIN users u ON u.id=d.owner_id WHERE d.id=?').get(id))
})
...
router.patch('/:id', (req, res) => {
  ...
  getDb().prepare(`UPDATE documents SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  res.json(getDb().prepare('SELECT d.*, u.name AS owner_name FROM documents d JOIN users u ON u.id=d.owner_id WHERE d.id=?').get(req.params.id))
})
...
router.patch('/:id/archive', (req, res) => {
  ...
  getDb().prepare('UPDATE documents SET archived = ? WHERE id = ?').run(archived, req.params.id)
