...
function nextTicketNumber(db) {
  const row = db.prepare('SELECT MAX(number) AS n FROM tickets').get()
  return (row.n ?? 0) + 1
}

function withReplies(db, ticket) {
...
router.get('/tickets/:id', (req, res) => {
  ...
  res.json(withReplies(db, ticket))
})
...
router.post('/tickets/:id/replies', (req, res) => {
  ...
  if (!IS_ADMIN(req.user) && ticket.submitter_id !== req.user.id)
    return res.status(403).json({ error: 'Forbidden' })
  const id = randomUUID()
  db.prepare('INSERT INTO ticket_replies (id, ticket_id, author_id, body) VALUES (?,?,?,?)')
    .run(id, req.params.id, req.user.id, body)
  db.prepare("UPDATE tickets SET updated_at = datetime('now') WHERE id = ?").run(req.params.id)
  res.status(201).json(db.prepare('SELECT * FROM ticket_replies WHERE id = ?').get(id))
})
