...
router.get('/stats', (_req, res) => {
  ...
  const headcount = db.prepare(
    "SELECT COUNT(*) AS n FROM users WHERE active = 1 AND archived = 0"
  ).get().n

  const pending_leave = db.prepare(
    "SELECT COUNT(*) AS n FROM leave_requests WHERE status = 'pending'"
  ).get().n

  const by_dept = db.prepare(
    "SELECT department, COUNT(*) AS n FROM users WHERE active = 1 AND archived = 0 GROUP BY department"
  ).all()

  const pending_list = db.prepare(`
...
router.patch('/expenses/:id', (req, res) => {
  ...
  const db = getDb()
  db.prepare('UPDATE expense_claims SET status = ? WHERE id = ?').run(status, req.params.id)
  res.json(db.prepare(`
    SELECT e.*, u.name AS employee_name, u.department AS employee_dept
...
router.get('/employees/:id/profile', (req, res) => {
  const db   = getDb()
  const user = db.prepare('SELECT id, name, email, department, role FROM users WHERE id = ?').get(req.params.id)
  if (!user) return res.status(404).json({ error: 'User not found' })
  const profile = db.prepare('SELECT * FROM employee_profiles WHERE user_id = ?').get(req.params.id)
  res.json({ ...user, profile: profile || {} })
})

router.put('/employees/:id/profile', (req, res) => {
  const { phone, address, emergency_name, emergency_phone, iban, personal_email } = req.body
  const db     = getDb()
  const exists = db.prepare('SELECT id FROM employee_profiles WHERE user_id = ?').get(req.params.id)
  if (exists) {
  ...
  res.json(db.prepare('SELECT * FROM employee_profiles WHERE user_id = ?').get(req.params.id))
})
...
router.post('/employees', (req, res) => {
  ...
  try {
    const user = createUser({ name, email, password, department, role })
...
router.get('/open-roles', (_req, res) => {
  res.json(getDb().prepare('SELECT * FROM open_roles ORDER BY created_at DESC').all())
})

router.post('/open-roles', (req, res) => {
  const { title, team, type = 'Full-time' } = req.body
  if (!title || !team) return res.status(400).json({ error: 'title and team required' })
  const db = getDb()
  const id = randomUUID()
  db.prepare('INSERT INTO open_roles (id, title, team, type) VALUES (?,?,?,?)').run(id, title, team, type)
  res.status(201).json(db.prepare('SELECT * FROM open_roles WHERE id = ?').get(id))
})

router.patch('/open-roles/:id', (req, res) => {
  ...
  db.prepare(`UPDATE open_roles SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  res.json(db.prepare('SELECT * FROM open_roles WHERE id = ?').get(req.params.id))
})
...
router.patch('/referrals/:id', (req, res) => {
  ...
  res.json(db.prepare('SELECT * FROM referrals WHERE id = ?').get(req.params.id))
})

// ── Payroll export (CSV) ──────────────────────────────────────────────────────

router.get('/payroll/export', (req, res) => {
  const db      = getDb()
...
router.post('/payroll/run', (req, res) => {
  ...
  const exists = db.prepare('SELECT COUNT(*) AS n FROM payroll_slips WHERE period = ?').get(period).n
  if (exists) return res.status(409).json({ error: `Payroll for "${period}" already exists` })

  const employees = db.prepare("SELECT id FROM users WHERE active = 1 AND archived = 0").all()
  const ins = db.prepare('INSERT INTO payroll_slips (id, employee_id, period, gross, net, status) VALUES (?,?,?,?,?,?)')
  const tx  = db.transaction(() => employees.forEach(e => ins.run(randomUUID(), e.id, period, 0, 0, 'draft')))
  tx()
...
router.patch('/payroll/slips/:id', (req, res) => {
  ...
  if (!sets.length) return res.status(400).json({ error: 'Nothing to update' })
  vals.push(req.params.id)
  db.prepare(`UPDATE payroll_slips SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
  res.json(db.prepare('SELECT * FROM payroll_slips WHERE id = ?').get(req.params.id))
})
