...
export function login(email, password) {
  const db   = getDb()
  const user = db.prepare('SELECT * FROM users WHERE email = ? AND active = 1 AND archived = 0').get(email)
  if (!user) return null
...
export function verifyToken(token) {
  try {
    ...
    const user = db.prepare('SELECT * FROM users WHERE id = ? AND active = 1 AND archived = 0').get(session.user_id)
    if (!user) return null
...
export function createUser({ name, email, password, department, role }) {
  const db   = getDb()
  const id   = randomUUID()
