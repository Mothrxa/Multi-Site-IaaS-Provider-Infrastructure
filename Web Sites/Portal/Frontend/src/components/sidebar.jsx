...
const SIDEBAR_W = 244;
const SIDEBAR_W_ICON = 64;

const NAV_IT = [
  { id: "overview",       label: "Overview",       icon: <I.grid size={17}/> },
  { kind: "section",      label: "Engineering" },
  { id: "development",    label: "Development",    icon: <I.branch size={17}/> },
  { id: "infrastructure", label: "Infrastructure", icon: <I.server size={17}/> },
  { kind: "section",      label: "Cloud Platform" },
  { id: "workloads",      label: "Workloads",      icon: <I.cpu size={17}/> },
  { kind: "section",      label: "Support" },
  { id: "tickets",        label: "Tickets",        icon: <I.ticket size={17}/>, liveKey: "tickets" },
  { id: "subscribers",    label: "Customers",      icon: <I.users size={17}/> },
  { kind: "section",      label: "Resources" },
  { id: "documents",      label: "Documents",      icon: <I.doc size={17}/> },
];

const NAV_HR = [
  { id: "overview",    label: "Overview",       icon: <I.grid size={17}/> },
  { kind: "section",   label: "People" },
  { id: "directory",   label: "Directory",      icon: <I.users size={17}/> },
  { id: "recruitment", label: "Recruitment",    icon: <I.briefcase size={17}/>, liveKey: "recruitment" },
  { id: "leave",       label: "Leave Requests", icon: <I.cal size={17}/>,       liveKey: "leave" },
  { kind: "section",   label: "Administration" },
  { id: "payroll",     label: "Payroll",        icon: <I.cash size={17}/> },
  { id: "expenses",    label: "Expenses",       icon: <I.cash size={17}/>,      liveKey: "expenses" },
  { id: "documents",   label: "Documents",      icon: <I.doc size={17}/> },
];
...
const NAV_BIZ = [
  ...
  { id: "customers", label: "Customers",      icon: <I.users size={17}/> },
  { id: "billing",   label: "Billing & Usage",icon: <I.cash size={17}/> },
  { id: "invoices",  label: "Invoices",       icon: <I.doc size={17}/> },
  { kind: "section", label: "Commercial" },
  { id: "revenue",   label: "Revenue",        icon: <I.chart size={17}/> },
  { id: "pipeline",  label: "Pipeline",       icon: <I.target size={17}/> },
  { kind: "section", label: "Resources" },
  { id: "documents", label: "Documents",      icon: <I.doc size={17}/> },
];
...
function Sidebar({ dept, current, onNav, mode = "labeled", logoVariant }) {
  ...
  return (
    <aside style={{
        ...
        {nav.map((item, i) => {
          if (item.kind === "section") {
            return iconOnly ? <div key={i} style={{ height: 14 }}/> : (
              <div key={i} style={{
                ...
                textTransform: "uppercase", padding: "14px 12px 6px", fontWeight: 600,
              }}>{item.label}</div>
            );
          }
          ...
          return (
            <button key={item.id} onClick={() => onNav(item.id)} style={{
              ...
              padding: iconOnly ? "10px" : "9px 12px",
      ...
      {/* Footer */}
      <div style={{ borderTop: "0.5px solid var(--hairline)", paddingTop: 10, display: "flex", flexDirection: "column", gap: 2 }}>
        {!iconOnly && (
          <div style={{ padding: "0 6px 8px", fontSize: 10.5, color: "var(--text-mute)", lineHeight: 1.5 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>Build</span>
              <span className="mono">26.05.19-b</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span>Region</span>
              <span className="mono">eu-cen-1</span>
            </div>
          </div>
        )}
