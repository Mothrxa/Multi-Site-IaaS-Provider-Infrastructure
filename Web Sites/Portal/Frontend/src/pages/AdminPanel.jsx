...
export function AdminPanel({ user, onBack }) {
  ...
  const [userFilter,   setUserFilter]   = React.useState('active') // 'active' | 'archived'
  ...
  function handleArchiveUser(u) {
    apiReq(`/api/users/${u.id}`, { method: 'PATCH', body: JSON.stringify({ archived: 1, active: 0 }) })
      .then(updated => setUsers(prev => prev.map(x => x.id === updated.id ? updated : x)))
  }

  function handleRestoreUser(u) {
    apiReq(`/api/users/${u.id}`, { method: 'PATCH', body: JSON.stringify({ archived: 0, active: 1 }) })
      .then(updated => setUsers(prev => prev.map(x => x.id === updated.id ? updated : x)))
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '28px 32px 60px', maxWidth: 1100, width: '100%', margin: '0 auto' }}>
        ...
        <div style={{ marginTop: 20 }}>
          {tab === 'users' && (() => {
            const filtered = users.filter(u => userFilter === 'archived' ? u.archived : !u.archived)
            return (
            <Card padding={0}>
              <div style={{ padding: '10px 14px', borderBottom: '0.5px solid var(--hairline)', display: 'flex', alignItems: 'center', gap: 10 }}>
                <TabBar dense
                  tabs={[
                    { id: 'active',   label: `Active (${users.filter(u => !u.archived).length})` },
                    { id: 'archived', label: `Archived (${users.filter(u => u.archived).length})` },
                  ]}
                  active={userFilter} onChange={setUserFilter}
                />
              </div>
              {loading ? (
                ...
                <table className="tbl" style={{ width: '100%' }}>
                  ...
                  <tbody>
                    ...
                    {filtered.map(u => (
                      <tr key={u.id} style={{ opacity: u.archived ? 0.6 : 1 }}>
                        <td>
                        ...
                        <td style={{ color: 'var(--text-dim)', fontFamily: 'var(--f-mono)', fontSize: 12 }}>{u.email}</td>
                        <td><Pill>{DEPT_LABELS[u.department] ?? u.department}</Pill></td>
                        <td><Pill tone={ROLE_TONES[u.role]}>{ROLE_LABELS[u.role] ?? u.role}</Pill></td>
                        <td>
                          <Pill tone={u.archived ? undefined : u.active ? 'good' : undefined}>
                            <span className="dot"/>
                            {u.archived ? 'Archived' : u.active ? 'Active' : 'Disabled'}
                          </Pill>
                        </td>
                        <td>
                          <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                            ...
                            {!u.archived && (
                              <button
                                className="btn ghost" style={{ height: 28, padding: '0 10px', fontSize: 12,
                                  color: u.active ? 'var(--bad)' : 'var(--good)' }}
                            ...
                            {u.archived ? (
                              <button
                                className="btn ghost" style={{ height: 28, padding: '0 10px', fontSize: 12, color: 'var(--good)' }}
                              ...
                              <button
                                className="btn ghost" style={{ height: 28, padding: '0 10px', fontSize: 12, color: 'var(--text-mute)' }}
...
function UserModal({ initial, onSave, onClose }) {
  ...
  const [dept,   setDept]   = React.useState(initial?.department ?? 'it')
  const [role,   setRole]   = React.useState(initial?.role       ?? 'employee')

  function handleSubmit(e) {
    e.preventDefault()
    const payload = { name, email, department: dept, role }
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <form onClick={e => e.stopPropagation()} onSubmit={handleSubmit} style={{
        ...
        <Field label="Email">
          <input className="input" style={{ width: '100%' }} type="email" value={email} onChange={e => setEmail(e.target.value)} required disabled={isEdit}/>
        </Field>
        <Field label={isEdit ? 'New password (leave blank to keep)' : 'Password'}>
          <input className="input" style={{ width: '100%' }} type="password" value={pass} onChange={e => setPass(e.target.value)} required={!isEdit} autoComplete="new-password"/>
        </Field>
