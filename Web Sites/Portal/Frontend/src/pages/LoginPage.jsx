...
export function LoginPage({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.message === 'Invalid credentials' ? 'Incorrect email or password.' : 'Could not connect to server.')
      setLoading(false)
    }
  }
    ...
    <div style={{
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
          <StrataLogo size={36} variant="stack"/>
          <div>
            <div style={{ fontWeight: 700, letterSpacing: '0.18em', fontSize: 15, color: 'var(--text)' }}>STRATA</div>
            <div style={{ fontSize: 10, color: 'var(--text-mute)', letterSpacing: '0.28em', textTransform: 'uppercase', marginTop: 2 }}>Cloud · Network</div>
          </div>
        </div>

        <div>
          <div style={{ fontSize: 20, fontWeight: 600, letterSpacing: '-0.02em', color: 'var(--text)' }}>Sign in</div>
          <div style={{ fontSize: 13, color: 'var(--text-mute)', marginTop: 4 }}>Use your STRATA account credentials</div>
        </div>
