...
export const resourcesApi = {
  ...
  stats:  (id)   => req(`/api/resources/${id}/stats`),
}

// WebSocket URL for the in-browser console — token goes in the query string
// since browsers can't set custom headers on the WS handshake.
export function consoleWsUrl(id) {
  const token = localStorage.getItem('strata_cloud_token')
  const httpBase = BASE || window.location.origin
  const wsBase = httpBase.replace(/^http/, 'ws')
  return `${wsBase}/ws/console?id=${encodeURIComponent(id)}&token=${encodeURIComponent(token || '')}`
}
