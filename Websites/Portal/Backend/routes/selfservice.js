...
router.patch('/leave/:id', async (req, res) => {
  const db  = getDb()
  const req_row = await db.prepare('SELECT * FROM leave_requests WHERE id = ?').get(req.params.id)
  ...
  await db.prepare(`UPDATE leave_requests SET ${sets.join(', ')} WHERE id = ?`).run(...vals)
...
router.post('/payslips', async (req, res) => {
  ...
  res.status(201).json(await db.prepare('SELECT * FROM payroll_slips WHERE id = ?').get(id))
})
...
router.get('/profile', async (req, res) => {
  const db  = getDb()
  const row = await db.prepare('SELECT * FROM employee_profiles WHERE user_id = ?').get(req.user.id)
  const user = await db.prepare('SELECT id, name, email, department FROM users WHERE id = ?').get(req.user.id)
...
router.put('/profile', async (req, res) => {
  ...
  const db  = getDb()
  const exists = await db.prepare('SELECT id FROM employee_profiles WHERE user_id = ?').get(req.user.id)
  if (exists) {
  ...
  res.json(await db.prepare('SELECT * FROM employee_profiles WHERE user_id = ?').get(req.user.id))
})
...
router.post('/expenses', async (req, res) => {
  ...
  res.status(201).json(await db.prepare('SELECT * FROM expense_claims WHERE id = ?').get(id))
})
