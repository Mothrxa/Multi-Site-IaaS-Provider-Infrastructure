...
import { usePortalRouter, pathFromState } from './router.js'
...
function App() {
  ...
  const [urlState, navigateUrl] = usePortalRouter()

  // Verify stored token on mount
  React.useEffect(() => {
    const token = localStorage.getItem('strata_token')
    const user  = localStorage.getItem('strata_user')
    if (!token || !user) {
      navigateUrl({ page: 'login' }, { replace: true })
      return
    }
    const parsed = (() => { try { return JSON.parse(user) } catch { return null } })()
    if (!parsed) { navigateUrl({ page: 'login' }, { replace: true }); return }

    authApi.me().then(() => {
      setAuth(parsed)
      // If on login path, redirect to portal
      if (window.location.pathname === '/' || window.location.pathname === '/login') {
        const dept = parsed.dept || 'it'
        const page = SUPER_ROLES.includes(parsed.role) ? 'admin' : 'portal'
        navigateUrl({ page, dept, route: 'overview' }, { replace: true })
      }
    }).catch(() => {
  ...
  function handleLogin(user) {
    ...
    if (SUPER_ROLES.includes(user.role)) {
      navigateUrl({ page: 'admin' })
    } else {
      navigateUrl({ page: 'portal', dept: user.dept || 'it', route: 'overview' })
    }
  }
  function handleLogout() {
    authApi.logout()
    setAuth(null)
    navigateUrl({ page: 'login' })
  }

  if (urlState.page === 'login' || !auth) return <LoginPage onLogin={handleLogin}/>
  if (urlState.page === 'admin')          return <AdminPanel user={auth} onBack={() => navigateUrl({ page: 'portal', dept: auth.dept || 'it', route: 'overview' })}/>
  ...
  return (
    <PortalApp
      ...
      onAdmin={() => navigateUrl({ page: 'admin' })}
      urlState={urlState}
...
function PortalApp({ auth, isAdmin, canSwitchDept, onLogout, onAdmin, urlState, navigateUrl }) {
  const [t, setTweak] = useTweaks(TWEAK_DEFAULTS);
  const [dept,   setDept]  = React.useState(urlState.dept || auth.dept || "it");
  const [route,  setRoute] = React.useState(urlState.route || "overview");
  const [shared, setShared] = React.useState(urlState.shared || null);
  ...
  React.useEffect(() => {
    setRoute("overview");
    navigateUrl({ page: 'portal', dept: effectiveDept, route: 'overview' }, { replace: true });
  }, [effectiveDept]);

  const goto = (r) => {
    setRoute(r);
    navigateUrl({ page: 'portal', dept: effectiveDept, route: r });
  };
  ...
  return (
    <div data-dept={effectiveDept} style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        <Sidebar dept={effectiveDept} current={route} onNav={goto} mode={t.sidebar} logoVariant={t.logo}/>
      ...
      {resetPassOpen && (
        <PasswordResetModal onClose={() => setResetPassOpen(false)}/>
      )}
...
function RouteView({ dept, route, goto, user, t, setTweak, onOpenShared, onComposeMailTo }) {
  ...
  if (dept === "it") {
    switch (route) {
      case "overview":      return <ITOverview goto={goto} user={user}/>;
...
function PasswordResetModal({ onClose }) {
  ...
  function handleSubmit() {
    ...
    fetch(`${BASE}/api/auth/change-password`, {
      ...
      .then(() => { setSuccess(true); setTimeout(onClose, 1500) })
      .catch(err => { setError(err.message); setLoading(false) })
  }

  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(30,24,14,0.28)', WebkitBackdropFilter: 'blur(10px)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} className="fadein">
      <div onClick={e => e.stopPropagation()} style={{ width: 380, background: 'var(--bg-2)', border: '0.5px solid var(--hairline-2)', borderRadius: 18, boxShadow: 'var(--shadow-pop)', padding: 26, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, fontSize: 16 }}>Reset password</h3>
          <button className="btn ghost icon" onClick={onClose}><I.x size={15}/></button>
        </div>
        {success ? (
          <div style={{ padding: '16px 0', textAlign: 'center', color: 'var(--good)', fontSize: 14 }}>✓ Password updated</div>
        ) : (<>
          {[
            ...
            <div key={f.label} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 500 }}>{f.label}</label>
              <input className="input" type="password" style={{ width: '100%' }} value={f.value} onChange={e => f.set(e.target.value)}/>
            </div>
          ))}
          {error && <div style={{ fontSize: 12, color: 'var(--bad)' }}>{error}</div>}
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button className="btn" onClick={onClose}>Cancel</button>
            <button className="btn primary" onClick={handleSubmit} disabled={loading || !current || !next || !confirm}>
