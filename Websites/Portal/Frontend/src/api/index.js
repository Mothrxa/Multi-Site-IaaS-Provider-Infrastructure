...
export const clientsApi = {
  stats:     ()      => req('/api/clients/stats'),
  list:      ()      => req('/api/clients'),
  get:       (id)    => req(`/api/clients/${id}`),
  create:    (data)  => req('/api/clients', { method: 'POST', body: JSON.stringify(data) }),
  update:    (id, d) => req(`/api/clients/${id}`, { method: 'PATCH', body: JSON.stringify(d) }),
  resources: (id)    => req(`/api/clients/${id}/resources`),
  billing:   (query) => req(`/api/clients/billing/all${query ? '?'+query : ''}`),
  pipeline:  ()      => req('/api/clients/pipeline/deals'),
}

export const itApi = {
  cloudStats: ()           => req('/api/it/cloud-stats'),
  workloads:  (params='') => req(`/api/it/workloads${params ? '?'+params : ''}`),
  customers:  ()           => req('/api/it/customers'),
}
