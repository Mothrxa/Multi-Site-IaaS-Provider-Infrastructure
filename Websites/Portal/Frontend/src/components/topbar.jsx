...
import { StrataLogo, CloudLogo } from './brand.jsx'
...
function TopBar({ dept, onDeptChange, canSwitchDept, theme, onThemeToggle, onOpenShared, sharedActive, sidebarMode, logoVariant, user, isAdmin, onAdmin, onLogout, onProfile, onResetPassword }) {
  ...
  React.useEffect(() => {
    ...
    // Refresh immediately when a message is read
    window.addEventListener('mail-read', fetchCounts)
    return () => { clearInterval(t); window.removeEventListener('mail-read', fetchCounts) }
  }, [])
  ...
  return (
    <header style={{
      ...
      {/* Brand */}
      <div style={{ display: "flex", alignItems: "center", gap: 12, paddingRight: 22, marginRight: 6, borderRight: "0.5px solid var(--hairline)", height: 56 }}>
        <CloudLogo size={48}/>
        <span style={{ fontWeight: 700, letterSpacing: "0.18em", fontSize: 18 }}>STRATA</span>
      </div>
      ...
      {canSwitchDept
        ...
        : <div style={{
            display: "flex", alignItems: "center", gap: 8, height: 34, padding: "0 12px 0 10px",
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 1, marginLeft: 4, marginRight: 2 }}>
        <span style={{ fontSize: 13, color: "var(--text)", fontWeight: 500 }} className="mono">{fmtTime}</span>
        <span style={{ fontSize: 11, color: "var(--text-mute)" }}>{fmtDate}</span>
      </div>

      {/* User */}
      <button onClick={() => setUserOpen(o => !o)} style={{
        display: "flex", alignItems: "center", gap: 9, padding: "4px 5px 4px 12px",
      ...
      {/* Modals */}
      {userOpen && <UserMenu user={user} theme={theme} onThemeToggle={onThemeToggle} isAdmin={isAdmin} onAdmin={onAdmin} onLogout={onLogout} onProfile={onProfile} onResetPassword={onResetPassword} onClose={() => setUserOpen(false)}/>}
    </header>
  );
}
...
function SharedMenuButton({ item, active, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      height: 44, borderRadius: 10, border: "0.5px solid transparent",
      background: active ? "var(--surface-3)" : "transparent",
      borderColor: active ? "var(--hairline)" : "transparent",
      color: active ? "var(--text)" : "var(--text-dim)",
      cursor: "pointer", fontSize: 16, fontFamily: "var(--f-sans)",
...
function UserMenu({ user, theme, onThemeToggle, isAdmin, onAdmin, onLogout, onProfile, onResetPassword, onClose }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 100 }}>
      <div onClick={e => e.stopPropagation()} className="fadein" style={{
        position: "absolute", right: 14, top: 60, width: 260, padding: 6,
        background: "var(--bg-2)", border: "0.5px solid var(--hairline-2)",
        ...
        <div style={{ borderTop: "0.5px solid var(--hairline)", paddingTop: 6 }}>
          ...
          <UserMenuItem icon={<I.lock size={14}/>} label="Reset password"
