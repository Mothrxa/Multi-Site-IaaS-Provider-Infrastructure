...
function ITSubscribers() {
  ...
  const totalWorkloads = (clients||[]).reduce((s,c) => s+(c.vm_count||0)+(c.container_count||0), 0);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "var(--gap-grid)" }}>
        ...
        <Card><Stat label="Trial" value={stats ? stats.trial||0 : '…'}/></Card>
        <Card><Stat label="At risk" value={stats ? stats.at_risk||0 : '…'} deltaTone="warn"/></Card>
        <Card><Stat label="Provisioned workloads" value={clients ? totalWorkloads.toLocaleString() : '…'}/></Card>
      </div>

      <Card title="Customer accounts" subtitle="All cloud clients"
            action={<input className="input" placeholder="Search customer…" style={{ width: 260 }} value={search} onChange={e=>setSearch(e.target.value)}/>}>
        <table className="tbl">
          <thead><tr><th>Account</th><th>Plan</th><th>Region</th><th>VMs</th><th>Containers</th><th>Status</th><th></th></tr></thead>
          <tbody>
              ...
              <tr key={c.id}>
                <td>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div><div style={{ fontWeight: 500 }}>{c.name}</div><div style={{ fontSize:11, color:'var(--text-mute)' }}>{c.company}</div></div>
                  </div>
                </td>
                <td><Pill>{PLAN_LABELS[c.plan]||c.plan}</Pill></td>
                <td className="mono" style={{ color: "var(--text-dim)" }}>{c.region||'DC-Alger'}</td>
