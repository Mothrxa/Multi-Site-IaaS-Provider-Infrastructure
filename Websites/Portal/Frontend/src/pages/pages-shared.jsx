...
function MailPage({ user, composeTo }) {
  ...
  const [composeInit, setComposeInit] = React.useState({});

  React.useEffect(() => { if (composeTo) { setComposeInit({ to: composeTo }); setCompose(true); } }, [composeTo]);

  function fetchFolder(f) {
    setLoading(true);
    setOpenMsg(null);
    mailApi.list(f)
  ...
  React.useEffect(() => { fetchFolder(folder); }, [folder]);

  function openMessage(msg) {
    // Open draft in compose instead of reader
    if (folder === "draft") {
      mailApi.get(msg.uid, "draft").then(full => {
        setComposeInit({ to: full.to || "", subject: full.subject || "", body: full.body || "", draftUid: msg.uid });
        setCompose(true);
      });
      return;
    }
    mailApi.get(msg.uid, folder).then(full => {
      ...
      if (msg.unread) {
        ...
        // Notify TopBar to refresh unread count
  ...
  function handleStar(msg) {
    const isStarred = folder === "star";
    mailApi.star(msg.uid, folder, !isStarred)
      .then(() => {
        setMessages(prev => prev.filter(m => m.uid !== msg.uid));
        if (openMsg?.uid === msg.uid) setOpenMsg(null);
      });
  }

  function handleDelete(uid) {
    mailApi.delete(uid, folder).then(() => setMessages(prev => prev.filter(m => m.uid !== uid)));
    if (openMsg?.uid === uid) setOpenMsg(null);
  }

  function handleRefresh() { fetchFolder(folder); }

  function handleSent() {
    setCompose(false);
    setComposeInit({});
    if (folder === "sent") fetchFolder("sent");
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18, height: "100%" }}>
            ...
            {messages.map(m => (
              <button key={m.uid} onClick={() => openMessage(m)} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: m.unread ? 600 : 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {folder === "sent" ? (m.to || m.from) : m.from}
                  </span>
                ...
                {folder === "draft" && (
                  <button onClick={e => { e.stopPropagation(); handleDelete(m.uid); }} className="btn ghost icon"
                    style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", width: 26, height: 26, color: "var(--bad)" }}>
                    <I.x size={13}/>
                  </button>
                )}
              </button>
            ...
            <>
              <div style={{ padding: 22, borderBottom: "0.5px solid var(--hairline)" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <Avatar name={folder === "sent" ? (opened.to || opened.from) : opened.from} size={36}/>
                  <div style={{ flex: 1 }}>
                    {folder === "sent" ? (
                      <div style={{ fontSize: 13, fontWeight: 500 }}>
                        <span style={{ color: "var(--text-mute)", fontWeight: 400 }}>To: </span>{opened.to}
                      </div>
                    ) : (
                    ...
                    <div style={{ fontSize: 11, color: "var(--text-mute)" }}>
                      {folder === "sent" ? opened.time : `to ${user.name} · ${opened.time}`}
                    </div>
                  </div>
                  <button className="btn ghost icon" onClick={() => handleStar(opened)}
                    style={{ color: folder === "star" ? "var(--warn)" : "var(--text-dim)" }}>
                    <I.star size={14}/>
                  </button>
              ...
              <div style={{ padding: 14, borderTop: "0.5px solid var(--hairline)", display: "flex", gap: 8 }}>
                <button className="btn primary" style={{ flex: 1 }} onClick={() => { setComposeInit({ to: opened.fromEmail, subject: `RE: ${opened.subject}` }); setCompose(true); }}><I.send size={14}/>Reply</button>
                <button className="btn" style={{ flex: 1 }} onClick={() => { setComposeInit({ subject: `FWD: ${opened.subject}` }); setCompose(true); }}>Forward</button>
              </div>
            </>
          )}
        </Card>
      ...
      {compose && <ComposeMail onClose={() => { setCompose(false); setComposeInit({}); }} onSent={handleSent} defaultTo={composeInit.to || ""} defaultSubject={composeInit.subject || ""} defaultBody={composeInit.body || ""}/>}
    </div>
  );
}

function ComposeMail({ onClose, onSent, defaultTo = "", defaultSubject = "", defaultBody = "" }) {
  ...
  function handleSend() {
    ...
    mailApi.send(to.trim(), subject.trim(), body)
      .then(() => { onSent?.(); })
      .catch(err => { setError(err.message || "Send failed"); setSending(false); });
  }

  function handleSaveDraft() {
    setSaving(true);
    mailApi.saveDraft(to, subject, body)
      .then(() => { setSaved(true); setTimeout(() => setSaved(false), 2000); })
      .catch(() => {})
      .finally(() => setSaving(false));
  }

  // Solid compose window — no glass/blur
  return (
    <div style={{
      position: "fixed", right: 24, bottom: 0, width: 520, zIndex: 200,
      ...
      boxShadow: "0 -4px 32px rgba(0,0,0,0.45), 0 0 0 0.5px rgba(255,255,255,0.05) inset",
      overflow: "hidden",
    }}>
      {/* Header */}
      <div style={{
        ...
        <button className="btn ghost icon" style={{ width: 26, height: 26 }} onClick={onClose}><I.x size={14}/></button>
      </div>

      {/* Fields */}
      <div style={{ borderBottom: "0.5px solid var(--hairline)", padding: "0 14px" }}>
        <input value={to} onChange={e => setTo(e.target.value)} placeholder="To"
          style={{ width: "100%", background: "transparent", border: "none", outline: "none",
            padding: "9px 0", fontSize: 13, color: "var(--text)", fontFamily: "var(--f-sans)",
      ...
      <textarea value={body} onChange={e => setBody(e.target.value)}
        ...
        style={{
          flex: 1, minHeight: 220, padding: "14px", resize: "none",
          background: "transparent", border: "none", outline: "none",
          color: "var(--text)", fontFamily: "var(--f-sans)", fontSize: 13.5, lineHeight: 1.6,
        }}/>

      {/* Error */}
      {error && <div style={{ padding: "0 14px 8px", fontSize: 12, color: "var(--bad)" }}>{error}</div>}

      {/* Footer */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", borderTop: "0.5px solid var(--hairline)" }}>
        <button className="btn primary" onClick={handleSend} disabled={sending || !to.trim() || !subject.trim()}>
          <I.send size={14}/>{sending ? "Sending…" : "Send"}
        </button>
        <button className="btn ghost" style={{ height: 30, padding: "0 10px", fontSize: 12 }} onClick={handleSaveDraft} disabled={saving}>
          {saved ? "✓ Saved" : saving ? "Saving…" : "Save draft"}
        </button>
        <div style={{ flex: 1 }}/>
        <button className="btn ghost icon" style={{ width: 30, height: 30, color: "var(--bad)" }} onClick={onClose}
          title="Discard"><I.archive size={14}/></button>
      </div>
    </div>
  );
}
